                   ...
                   EntryClassesMap &entry_class_hash)
{
        ...
        tbb::filter::parallel, [&](const tbb::blocked_range<NodeID> &intersection_node_range) {
                 ...
                 ++intersection_node)
            {
                ...
                const auto &incoming_edges =
                    extractor::intersection::getIncomingEdges(node_based_graph, intersection_node);
                const auto &outgoing_edges =
                    extractor::intersection::getOutgoingEdges(node_based_graph, intersection_node);
                const auto &edge_geometries_and_merged_edges =
                    extractor::intersection::getIntersectionGeometries(node_based_graph,
                                                                       compressed_edge_container,
                                                                       node_coordinates,
                ...
                for (const auto &incoming_edge : incoming_edges)
                {
                    ...
                    {
                        if (!extractor::intersection::isTurnAllowed(node_based_graph,
                                                                    edge_based_node_container,
                                                                    node_restriction_map,
                                                                    barrier_nodes,
                                                                    edge_geometries,
    ...
    // util::Log() << "Generated " << turn_data_container.size() << " turns";
