...
operator()(const NodeID /*nid*/, const EdgeID source_edge_id, Intersection intersection) const
{
    ...
    for (std::size_t road_index = 1, last = intersection.size(); road_index < last; ++road_index)
    {
        ...
        // std::cout << "sliproad_edge: " << sliproad_edge << std::endl;
        auto index_of_sliproad = [&](const auto &road) {
            // std::cout << "road.eid: " << road.eid << std::endl;
            return road.eid == sliproad_edge;
        };
        auto index = std::find_if(begin(intersection), end(intersection), index_of_sliproad) -
                     begin(intersection);

        if (isThroughStreet(index,
                            intersection,
                            node_based_graph,
                            node_data_container,
                            name_table,
